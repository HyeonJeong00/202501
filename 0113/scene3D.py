...
class SceneReconstruction3D:
    ...
    def load_image_pair(
        ...
        self.img1, self.img2 = [cv2.undistort(path, self.K, self.d) for path in (img1, img2)]   # 렌즈 왜곡을 보정
...
#         return img
    ...
    def plot_optic_flow(self):
        ...
        for pt1, pt2 in zip(self.match_pts1, self.match_pts2):
            # float -> int 변환
            pt1 = tuple(map(int, pt1))  # pt1을 정수형 튜플로 변환
            pt2 = tuple(map(int, pt2))  # pt2를 정수형 튜플로 변환
            cv2.arrowedLine(img, pt1, pt2, color=(255, 0, 0), thickness=1)  # 화살표 모양의 직선을 그릴 때 사용
